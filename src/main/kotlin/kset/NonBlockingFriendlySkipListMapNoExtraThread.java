 ...
 *            The value
 */
public class NonBlockingFriendlySkipListMapNoExtraThread<K, V> extends AbstractMap<K, V>
	 ...
	 */
	private static final boolean maintenance = false;
	 ...
	 */
	private static final ConcurrentLinkedQueue<NonBlockingFriendlySkipListMapNoExtraThread> skipLists = new ConcurrentLinkedQueue<NonBlockingFriendlySkipListMapNoExtraThread>();
	...
	static final AtomicReferenceFieldUpdater<NonBlockingFriendlySkipListMapNoExtraThread, HeadPointer> topStartUpdater = AtomicReferenceFieldUpdater
			.newUpdater(NonBlockingFriendlySkipListMapNoExtraThread.class,
	...
	static final AtomicReferenceFieldUpdater<NonBlockingFriendlySkipListMapNoExtraThread, HeadPointer> bottomStartUpdater = AtomicReferenceFieldUpdater
			.newUpdater(NonBlockingFriendlySkipListMapNoExtraThread.class,
	...
	// Constructors
	public NonBlockingFriendlySkipListMapNoExtraThread() {
		this(null, 0);
	}

	public NonBlockingFriendlySkipListMapNoExtraThread(double maintPercentage) {
	 ...
	 */
	public NonBlockingFriendlySkipListMapNoExtraThread(Comparator<? super K> comparator,
...
//					.println("Height Changes: " + bottomLevelRaiseCount.get());
	...
	public static void doMaintenance() {
		while (!stop) {
			...
			for (int i = 0; i < arry.length; i++) {
				((NonBlockingFriendlySkipListMapNoExtraThread) arry[i]).mixedTraversal();
			}
		}
		if (STRUCT_MODS) {
			Object[] arry = (Object[]) skipLists.toArray();
			for (int i = 0; i < arry.length; i++) {
				((NonBlockingFriendlySkipListMapNoExtraThread) arry[i]).vars.removals = counts
						.get().structMods;
			}
		}
	...
	abstract class Iter<T> implements Iterator<T> {
		...
		public void remove() {
			...
			NonBlockingFriendlySkipListMapNoExtraThread.this.remove(l.key);
			lastReturned = null;
		}
	...
	static final class Values<E> extends AbstractCollection<E> {
		...
		public Iterator<E> iterator() {
			...
			return ((NonBlockingFriendlySkipListMapNoExtraThread<Object, E>) m)
					.valueIterator();
